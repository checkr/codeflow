...
func main() {
	flag.Parse()
	log.Println("Kubernetes Elasticsearch logging discovery")
	...
	if envNamespace != "" {
		if _, err := client.Core().Namespaces().Get(envNamespace, v1.GetOptions{}); err != nil {
			log.Fatalf("%s namespace doesn't exist: %v", envNamespace, err)
		}
	...
	for t := time.Now(); time.Since(t) < 5*time.Minute; time.Sleep(10 * time.Second) {
		elasticsearch, err = client.Core().Services(namespace).Get("elasticsearch-logging", v1.GetOptions{})
		if err == nil {
			break
		}
	}
	...
	if elasticsearch == nil {
		log.Printf("Failed to find the elasticsearch-logging service: %v\n", err)
		return
	}
	...
	count := 0
	for t := time.Now(); time.Since(t) < 5*time.Minute; time.Sleep(10 * time.Second) {
		endpoints, err = client.Core().Endpoints(namespace).Get("elasticsearch-logging", v1.GetOptions{})
		if err != nil {
			continue
		}
	...
	if err != nil {
		log.Printf("Error finding endpoints: %v\n", err)
		return
	}

	log.Printf("Endpoints = %s", addrs)
