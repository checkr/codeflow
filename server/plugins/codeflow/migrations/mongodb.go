...
import (
	...
	"github.com/checkr/codeflow/server/plugins/codeflow/migrations/mongodb_bongo"
...
func init() {
	gomethods.RegisterMethodsReceiverForDriver("mongodb_bongo", &MongoDbMigrator{})
}
...
func (r *MongoDbMigrator) V001_init_organizations_up(session *mgo.Session) error {
...
func (r *MongoDbMigrator) V001_init_organizations_down(session *mgo.Session) error {
	return session.DB(DB_NAME).C(ORGANIZATIONS_C).DropCollection()
}

func (r *MongoDbMigrator) V001_init_users_up(session *mgo.Session) error {
	users := []User{
...
func (r *MongoDbMigrator) V001_init_users_down(session *mgo.Session) error {
	return session.DB(DB_NAME).C(USERS_C).DropCollection()
}

func (r *MongoDbMigrator) V002_organizations_rename_location_field_to_headquarters_up(session *mgo.Session) error {
...
func (r *MongoDbMigrator) V002_organizations_rename_location_field_to_headquarters_down(session *mgo.Session) error {
	c := session.DB(DB_NAME).C(ORGANIZATIONS_C)
...
func (r *MongoDbMigrator) V002_change_user_cleo_to_cleopatra_up(session *mgo.Session) error {
...
func (r *MongoDbMigrator) V002_change_user_cleo_to_cleopatra_down(session *mgo.Session) error {
	c := session.DB(DB_NAME).C(USERS_C)
...
func (r *MongoDbMigrator) v001_not_exported_method_up(session *mgo.Session) error {
	return nil
}

func (r *MongoDbMigrator) V001_method_with_wrong_signature_up(s string) error {
	return nil
}

func (r *MongoDbMigrator) V001_method_with_wrong_signature_down(session *mgo.Session) (bool, error) {
	return true, nil
}
